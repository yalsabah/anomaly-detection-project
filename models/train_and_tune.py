...
def tune_isolation_forest(X_train_norm, val_norm, val_anom):
    ...
    y_tune = np.concatenate([
        np.zeros(len(X_train_norm), dtype=int),
        np.zeros(len(val_norm), dtype=int),
        np.ones(len(val_anom), dtype=int)
    ])
    print("Unique labels in y_tune:", np.unique(y_tune))
    ...
    def adjusted_precision(y_true, y_pred):
        # IsolationForest uses -1 for anomaly, but your labels use 1 for fraud
        # So we remap y_pred: -1 -> 1 (fraud), 1 -> 0 (normal)
        y_pred_mapped = (y_pred == -1).astype(int)
        return precision_score(y_true, y_pred_mapped, pos_label=1, average='binary', zero_division=0)

    precision_scorer = make_scorer(adjusted_precision)
    ...
    search = RandomizedSearchCV(
        ...
        refit=True,
        verbose=1,
        n_jobs=2  # Use 2 or 4 cores instead of all
    )
...
def tune_autoencoder(X_train, X_val):
    ...
    plt.ylabel('MSE Loss')
    plt.legend()
    plt.savefig('reports/autoencoder_loss_curve.png')
    plt.close()
...
def tune_lstm_autoencoder(X_train_seq, X_val_seq):
    ...
    plt.ylabel('MSE Loss')
    plt.legend()
    plt.savefig('reports/lstm_autoencoder_loss_curve.png')
    plt.close()
...
def evaluate_models(iso_model, ae_model, ae_thresh, lstm_model, lstm_thresh,
    ...
    plt.legend()
    plt.savefig('reports/precision_recall_curves.png')
    plt.close()
...
def main():
    ...
    X_test_scaled, y_train_full, y_test, scaler = load_and_preprocess_data('data/creditcard.csv')
